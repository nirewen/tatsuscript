...
const Parser = {
    parse: (tokens, stopToken = 'NONE', initialIndex = 0, argIndex = 0) => {
    	let end = [];
    	for (let i = initialIndex; i < tokens.length; i++) {
    		...
    		switch(token.type) {
    			case 'LBRACKET':
    				let bracketGroup = Parser.parse(tokens, ['RBRACKET', 'EOF'], i + 1);
    				...
    				break;
    			case 'SEMI':
    				let argsGroup = Parser.parse(tokens, ['SEMI', 'RBRACKET', 'EOF'], i + 1);
